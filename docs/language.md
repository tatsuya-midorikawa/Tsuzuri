...
| `for x in values { C }` | `B.For values (x -> C)` |
| `while condition { C }` | `B.While (u -> condition) (B.Delay (u -> C))` |
...
通常の式にはループ文がなく、カスタムコンピュテーション式の `for`／`while` はビルダー操作への展開です。

`if condition { a } else { b }` は条件が `bool`、両分岐は同じ型でなければなりません。
実行するのは選ばれた分岐だけですが、両方を型検査します。通常の式では `else` は必須です。
カスタムコンピュテーション式の文としての `if` だけは、ビルダーの `Zero` により省略できます。
...
レコード、配列、リスト、関数値、タスクはエクスポートできません。これらの型もモジュール内部・モジュール間では使用できます。
