...
整数表示・解析は5,000反復、UTF往復は要求長26,215（実際は32,768 UTF-16単位）です。
値は各回の中央値の中央値。**処理時間（ms）は小さいほど高速、改善倍率（前回 / 今回）は大きいほど改善しています。**
新旧は別プロセスなので、負荷やスケジューリングの差が残る点は初回と同じです。
...
整数表示・解析は約33%の時間短縮です。50反復・50,000反復でも改善を確認しました。
50反復の初回には他の実装を含め大きな揺れがあり、Tsuzuri も遅い値が出ましたが、追加2回では
前回約0.0054 msに対し今回約0.0036 msでした。この初回データも削除していません。
UTFの効果は数%なので、これだけで他環境でも速いとは判断しません。ASCII／混在 Unicode の結果を別々にも確認しています。

同条件の C++・Rust・C#・JavaScript 比較も35種目で再実行し、全チェックサムを照合しました。
次の表はその1回の実行の中央値で、上の3回集計とは別です。**処理時間（ms）は小さいほど高速です。**
...
整数表示・解析の C++ 比は約1.9から約1.26、C# 比は約2.1から約1.34に縮まりましたが、まだ遅れは残ります。
UTF往復も C++／Rust／C# を上回ってはいません。GPU・自動並列化・worker pool の追加による結果ではありません。

採用した変更:

- 全幅の整数表示を2桁ずつ処理し、64/128-bit除算を減らす。binary の正負ゼロは直接表示する。
- 十進整数を8桁ずつ検証・集約し、基数1億の範囲検査を行う。区切りや他の基数は従来の1桁処理へ戻す。
- 数値の読み書きを、型幅に合う非整列対応の固定サイズコピーへ下げる。範囲外の読み書きやポインターの別名制約は追加しない。
- 浮動小数点用の大きな作業領域を整数の表示・解析から分離する。最終 ARM64 の入口のスタックは表示約39 KiBから112バイト、解析約11 KiBから208バイトに縮小した。
- UTF の計数段階で完了した検証結果を、同じ不変入力の書き込み段階で再利用する。不正 UTF-8・孤立サロゲートの拒否と正確な確保サイズは維持する。

`tests/display_parse_runtime.c` に入出力・プロセス起動を時間に含めない反復測定も追加しました。
`F kind repeats low_hex high_hex` は表示、`P kind repeats ascii_hex` は解析で、出力は CPU時間（ms/回）とチェックサムです。
入力値の準備・結果の回収は両版で同じ計測区間に含みます。速度の合否閾値は設けていません。
型別には各3サンプル、短い値・整数は100万回、通常の浮動小数点は10万回ずつ確認しました。
通常の浮動小数点はほぼ同速、64/128-bit最大値の解析は約2.3〜2.8倍でした。
一方、8-bit の短い解析などには1〜数ナノ秒（1 ns = 0.000001 ms）の増加が残ります。
この小さな差も記録し、全入力での高速化は主張しません。

native/WASM `-O0`／`-O3` で表示88,063件・解析/往復111,282件、型変換1,585境界ケース、
decimal/binary128参照演算、全Unicodeスカラー、不正入力・所有値回収を検証しました。
追加した解析ケースは各8桁位置の全バイト値、全整数幅・符号、区切り、先頭ゼロ、桁数・範囲・長さ上限です。
数値の入出力は1バイトずらした領域と前後の番兵でも検査しています。
...
生データは `target/benchmarks/followup-20260926/published-{generic,native}-{before,after}-{1,2,3}.json`、
言語間比較は `published-managed.json`、型別は `numeric-isolated-final-code.json` に保存しました。
型別 JSON には再実行用のプロトコルと反復数、小／大サイズは `size-*` の JSON に残しています。
`before` は保存した前回のコンパイラ、`after` は今回のコンパイラです。通常の `run-cpp.mjs --scale` と `--cpu` で再測定できます。
