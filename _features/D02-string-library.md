...
> この提案の本文は UTF-8 の旧 string を前提とする未実装の設計です。
> 現行の [文字列仕様](../docs/language.md#string-と-utf8string) は UTF-16 の string と UTF-8 の utf8string を分離しています。
> バイト境界・UTF-8 妥当性に関する API は Utf8String 向けに読み替え、
> String 向けの検索・切り出しは孤立サロゲートを保持するコード単位規則で再設計してください。
> 下記の未実装 API や旧型名を現行実装の契約として扱わないでください。
